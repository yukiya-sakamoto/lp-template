          ...
          <div style={{ backgroundColor: "#fff", border: "1px solid #EDE0CC", borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ padding: "22px" }}>
              ...
              {pricing.insurance.map(row => (
                <div key={row.label} style={{ display: "grid", gridTemplateColumns: "1fr 90px 90px", borderBottom: "1px solid #F7F0E6" }}>
                  <div style={{ fontSize: 14, color: "#2D2D2D", fontWeight: 600, padding: "14px 0" }}>{row.label}</div>
                  <div style={{ fontSize: 17, color: "#D96B0B", fontWeight: 700, textAlign: "center", padding: "14px 0" }}>{row.col1}</div>
                  <div style={{ fontSize: 17, color: "#05AF4B", fontWeight: 700, textAlign: "center", padding: "14px 0" }}>{row.col2}</div>
                </div>
              ))}
          ...
          <div style={{ backgroundColor: "#fff", border: "1px solid #EDE0CC", borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ padding: "22px" }}>
              {pricing.extras.map(row => (
                <div key={row.label} style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #F7F0E6", padding: "11px 0" }}>
                  <span style={{ fontSize: 14, color: "#2D2D2D" }}>{row.label}</span>
                  <span style={{ fontSize: 14, color: "#D96B0B", fontWeight: 700 }}>{row.price}</span>
                </div>
              ))}
