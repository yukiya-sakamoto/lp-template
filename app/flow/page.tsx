    ...
    <>
      ...
      <main style={{ backgroundColor: "#FDFAF6", padding: "80px 28px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <h2 style={{ fontSize: "clamp(18px, 2.4vw, 26px)", fontWeight: 700, color: "#2D2D2D", marginBottom: 40, lineHeight: 1.5 }}>はじめての方も安心の流れ</h2>
            ...
            {c.flow.map((step, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 32, alignItems: "center", backgroundColor: "#fff", border: "1px solid var(--color-border)", borderRadius: 10, overflow: "hidden" }} className="grid-2">
                ...
                <div style={{ padding: "24px 28px 24px 0" }}>
                  <div style={{ fontSize: 17, fontWeight: 700, color: "#2D2D2D", marginBottom: 10 }}>{step.title}</div>
                  <p style={{ fontSize: 15, color: "#4A4A4A", lineHeight: 2.0 }}>{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 56, backgroundColor: "var(--color-primary-light)", border: "1px solid var(--color-primary-border)", borderRadius: 10, padding: "32px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: "#2D2D2D", marginBottom: 20 }}>お気軽にご来院ください</div>
